...
import { getMonitorTargets, getChatLogs, addChatTarget, removeChatTarget } from "@/app/actions/wa-monitor";
...
export default function ChatMonitorModal({ providerId, onClose }: { providerId: string, onClose: () => void }) {
  ...
  const handleExport = () => {
    const csvContent = "data:text/csv;charset=utf-8," 
      + "Date,Time,Direction,Peer Name,Peer Number,Sender,Text,Media\n"
      + filteredLogs.map(l => {
          const dt = new Date(l.timestamp);
          const targetObj = targets.find(t => t.id === l.targetId);
          const direction = l.isFromMe ? 'Outbound' : 'Inbound';
          const sender = l.isFromMe ? 'Omni Bot' : (targetObj?.targetName || 'Target');
          return `${dt.toLocaleDateString()},${dt.toLocaleTimeString()},${direction},${targetObj?.targetName || 'Unknown'},${targetObj?.phoneNumber || ''},${sender},${l.textContent?.replace(/,/g, ' ') || ''},${l.mediaUrl ? 'Yes' : 'No'}`;
        }).join("\n");
                    ...
                    <table className="w-full text-left text-sm text-zinc-300 print:text-black">
                      ...
                      <tbody className="divide-y divide-white/5 print:divide-gray-300">
                        {filteredLogs.map(l => {
                          const targetObj = targets.find(t => t.id === l.targetId);
                          return (
                            <tr key={l.id} className={`hover:bg-white/[0.02] ${l.isFromMe ? 'bg-blue-500/[0.02]' : ''}`}>
