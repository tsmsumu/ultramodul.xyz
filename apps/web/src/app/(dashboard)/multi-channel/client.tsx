...
import { toggleProviderActive, updateProviderConfig, createWhatsAppNode, createTelegramNode, createSignalNode } from "@/app/actions/multi-channel";
...
export default function MultiChannelDashboard({
  ...
  const renderLiveExplorer = () => {
    ...
    const sigProviders = initialProviders.filter(p => p.providerType === 'signal');
    const sigArchived = archivedProviders?.filter(p => p.providerType === 'signal') || [];
