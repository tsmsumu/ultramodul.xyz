...
import { getMonitorTargets, getChatLogs, addChatTarget, removeChatTarget, bulkDeleteLogs, bulkArchiveLogs, bulkDeleteTargets, importLogbookData } from "@/app/actions/email-monitor";
...
export default function EmailChatMonitorModal({ providerId, onClose }: { providerId: string, onClose: () => void }) {
  ...
  const fetchData = async () => {
    setLoading(true);
    const { chat } = await getMonitorTargets(providerId);
    setTargets(chat);
    const logData = await getChatLogs(providerId);
    setLogs(logData);
    setLoading(false);
  };
  ...
  const handleAddTarget = async (e: React.FormEvent) => {
    ...
    await addChatTarget(providerId, phone, name, isTextOnly);
    setPhone("");
    setName("");
    setIsTextOnly(false);
    fetchData();
  ...
  const handleRemoveTarget = async (id: string) => {
    await removeChatTarget(providerId, id);
    fetchData();
  };
  ...
  const handleBulkDelete = async () => {
    ...
    if (window.confirm(`Delete ${selectedRows.length} logs permanently?`)) {
      await bulkDeleteLogs('chat', selectedRows);
      setSelectedRows([]);
      fetchData();
    }
  };
  ...
  const handleImportData = async (data: any[]) => {
    setImporting(true);
    const res = await importLogbookData(providerId, 'chat', data);
    if (res.success) {
  ...
  const exportOptions = {
    ...
    columns: [
      ...
      { header: "Peer Name", key: "peerName" },
      { header: "Peer Number", key: "peerNumber" },
    ...
    data: filteredLogs.map(l => {
      ...
      return {
        ...
        direction: l.isFromMe ? 'Outbound' : 'Inbound',
        peerName: targetObj?.targetName || 'Unknown',
        peerNumber: targetObj?.phoneNumber || '',
        sender: l.isFromMe ? 'Omni Bot' : (targetObj?.targetName || 'Target'),
      ...
      <div className="bg-zinc-950 border border-white/10 rounded-3xl w-full max-w-5xl h-[85vh] flex flex-col overflow-hidden shadow-2xl print:h-auto print:border-none print:shadow-none">
        ...
        <div className="p-6 border-b border-white/10 flex justify-between items-center bg-zinc-900/50 print:hidden">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-white">Chat Monitor</h2>
              <p className="text-xs text-zinc-400">Direct Message 1-on-1 Intelligence Logbook</p>
            </div>
          </div>
        ...
        <div className="flex px-6 pt-4 border-b border-white/10 gap-6 print:hidden">
          <button 
            ...
            className={`pb-4 text-sm font-bold uppercase tracking-wider border-b-2 transition-colors ${activeTab === 'logs' ? 'border-blue-500 text-blue-400' : 'border-transparent text-zinc-500 hover:text-zinc-300'}`}
          >
            📖 Logbook
          </button>
          <button 
            onClick={() => setActiveTab('targets')}
            className={`pb-4 text-sm font-bold uppercase tracking-wider border-b-2 transition-colors ${activeTab === 'targets' ? 'border-blue-500 text-blue-400' : 'border-transparent text-zinc-500 hover:text-zinc-300'}`}
          >
            🎯 Chat Targets
          </button>
          <button 
        ...
        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar print:p-0 print:overflow-visible">
          {loading ? (
            <div className="h-full flex items-center justify-center text-blue-400 print:hidden">Loading Intel Data...</div>
          ) : (
            <>
              {/* DELIVERY TAB */}
              {activeTab === 'delivery' && (
              ...
              {/* ANALYTICS TAB */}
              {activeTab === 'analytics' && (
                <LogAnalytics logs={filteredLogs} targets={targets} type="emailChat" />
              )}
                  ...
                  <div className="bg-zinc-900/50 border border-white/5 rounded-2xl p-6">
                    ...
                    <form onSubmit={handleAddTarget} className="flex gap-4 items-end">
                      ...
                      <div className="flex-1 space-y-2">
                        ...
                        <input type="text" value={name} onChange={e=>setName(e.target.value)} placeholder="Mr. Target" className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:ring-1 focus:ring-blue-500 focus:outline-none" required />
                      </div>
                      <div className="flex items-center gap-2 mb-3">
                        <input type="checkbox" id="textOnly" checked={isTextOnly} onChange={e=>setIsTextOnly(e.target.checked)} className="w-4 h-4 rounded bg-black/40 border-white/10 text-blue-500 focus:ring-blue-500" />
                        <label htmlFor="textOnly" className="text-xs text-zinc-400">Text Only (No Media)</label>
                      </div>
                      <button type="submit" className="bg-blue-600 hover:bg-blue-500 text-white px-6 py-3 rounded-xl font-bold flex items-center gap-2 h-[46px]">
                    ...
                    <div className="p-4 border-b border-white/5 bg-zinc-900/30 flex flex-col gap-4">
                      <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                        {selectedTargetRows.length > 0 ? (
                          <div className="bg-blue-900/20 px-4 py-2 border border-blue-500/20 flex items-center gap-4 rounded-lg w-full justify-between">
                          ...
                          <div className="text-xs font-bold text-zinc-400 uppercase tracking-widest">
                            Private Chat Targets
                          </div>
                        )}
                      </div>
                      
                      <div className="relative w-full">
                        <Search className="w-5 h-5 absolute left-4 top-1/2 -translate-y-1/2 text-blue-500/50" />
                        <input 
                          type="text" 
                          placeholder="Cari target Private Chat berdasarkan Nama atau Nomor..." 
                          value={searchTargetQuery}
                          onChange={e=>setSearchTargetQuery(e.target.value)}
                          className="w-full bg-black/60 border border-blue-500/30 rounded-xl pl-12 pr-4 py-3 text-sm text-white focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all placeholder:text-zinc-600 shadow-inner"
                        />
                      </div>
                    </div>
                ...
                <div className="space-y-4">
                  <div className="flex flex-col gap-4 print:hidden">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div className="flex flex-wrap items-center gap-2">
                        <div className="flex bg-black/40 rounded-lg p-1 border border-white/5">
                          <button onClick={() => { setLogTab('active'); setSelectedRows([]); }} className={`px-4 py-1.5 rounded-md text-xs font-bold uppercase tracking-widest transition-colors ${logTab === 'active' ? 'bg-blue-600 text-white' : 'text-zinc-500 hover:text-white'}`}>Active</button>
                      ...
                      <div className="flex flex-wrap gap-2">
                        <ImportMenu type="emailChat" isLoading={importing} onImport={handleImportData} />
                        <ExportMenu options={exportOptions} />
                        <button onClick={handlePrint} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg text-sm font-bold transition-colors">
                    ...
                    <div className="relative w-full">
                      ...
                      <input 
                        type="text" 
                        placeholder="Ketik untuk mencari log pesan..." 
                        value={searchQuery}
                        onChange={e=>setSearchQuery(e.target.value)}
                        className="w-full bg-black/60 border border-blue-500/30 rounded-xl pl-12 pr-4 py-4 text-sm text-white focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all placeholder:text-zinc-600 shadow-inner"
                      />
                    </div>
                  </div>

                  {selectedRows.length > 0 && (
                    <div className="flex items-center justify-between bg-blue-500/10 border border-blue-500/30 px-4 py-2 rounded-xl print:hidden">
                      <span className="text-sm font-bold text-blue-400">{selectedRows.length} logs selected</span>
                      <div className="flex gap-2">
                        {logTab === 'active' ? (
                          ...
                          <button onClick={() => handleBulkArchive(false)} className="px-3 py-1 bg-blue-600 hover:bg-blue-500 text-white rounded text-xs font-bold">Unarchive Selected</button>
                        )}
                  ...
                  <div className="hidden print:block mb-8">
                    <h1 className="text-2xl font-bold text-black border-b border-black pb-2 mb-4">Laporan Pemantauan Pesan DM (Chat History)</h1>
                    ...
                    <table className="w-full text-left text-sm text-zinc-300 print:text-black">
                      <thead className="bg-zinc-900/80 text-xs uppercase text-zinc-500 print:bg-gray-100 print:text-black">
                        <tr>
                          ...
                          <th className="px-6 py-4">Peer Contact</th>
                      ...
                      <tbody className="divide-y divide-white/5 print:divide-gray-300">
                        {filteredLogs.map(l => {
                          const targetObj = targets.find(t => t.id === l.targetId);
                          return (
                            <tr key={l.id} className={`hover:bg-white/[0.02] ${l.isFromMe ? 'bg-blue-500/[0.02]' : ''} ${selectedRows.includes(l.id) ? 'bg-white/[0.05]' : ''}`}>
                              ...
                              <td className="px-6 py-4">
                                <div className="font-bold text-blue-400 uppercase tracking-wider mb-1 text-xs">{targetObj?.targetName || 'Unknown'}</div>
                                  ...
                                  <div className="flex flex-col gap-2 print:hidden">
                                    {l.mediaType === 'image' ? (
                                      <div className="relative group">
                                        <img src={l.mediaUrl} alt="Chat Media" className="w-24 h-24 object-cover rounded-lg border border-white/10" />
                        ...
                        {filteredLogs.length === 0 && (
                          <tr><td colSpan={4} className="px-6 py-8 text-center text-zinc-500 italic">No chat logs recorded yet.</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
