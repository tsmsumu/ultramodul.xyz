...
import { getMonitorTargets, getGroupLogs, addGroupTarget, removeGroupTarget, bulkDeleteLogs, bulkArchiveLogs, bulkDeleteTargets, importLogbookData } from "@/app/actions/email-monitor";
...
export default function EmailGroupMonitorModal({ providerId, onClose }: { providerId: string, onClose: () => void }) {
  ...
  const fetchData = async () => {
    setLoading(true);
    const { emailGroup } = await getMonitorTargets(providerId);
    setTargets(emailGroup);
  ...
  const handleAddTarget = async (e: React.FormEvent) => {
    ...
    // Group IDs usually end in @g.us
    let cleanId = groupId;
    if (!cleanId.includes('@g.us')) {
      cleanId = `${cleanId}@g.us`;
    }

    await addGroupTarget(providerId, cleanId, groupName, isTextOnly);
    setGroupId("");
    setGroupName("");
    setIsTextOnly(false);
    fetchData();
  ...
  const handleRemoveTarget = async (id: string) => {
    await removeGroupTarget(providerId, id);
    fetchData();
  };
  ...
  const handleBulkDelete = async () => {
    ...
    if (window.confirm(`Delete ${selectedRows.length} logs permanently?`)) {
      await bulkDeleteLogs('emailGroup', selectedRows);
      setSelectedRows([]);
      fetchData();
    }
  };
  ...
  const handleBulkArchive = async (archive: boolean) => {
    if (selectedRows.length === 0) return;
    await bulkArchiveLogs('emailGroup', selectedRows, archive);
    setSelectedRows([]);
    fetchData();
  };
  ...
  const handleBulkDeleteTargets = async () => {
    ...
    if (window.confirm(`Delete ${selectedTargetRows.length} WAG targets and all their associated logs? This action cannot be undone.`)) {
      await bulkDeleteTargets('emailGroup', selectedTargetRows);
  ...
  const handleImportData = async (data: any[]) => {
    setImporting(true);
    const res = await importLogbookData(providerId, 'emailGroup', data);
    if (res.success) {
      ...
      <div className="bg-zinc-950 border border-white/10 rounded-3xl w-full max-w-5xl h-[85vh] flex flex-col overflow-hidden shadow-2xl print:h-auto print:border-none print:shadow-none">
        ...
        <div className="p-6 border-b border-white/10 flex justify-between items-center bg-zinc-900/50 print:hidden">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-white">WAG Monitor</h2>
              <p className="text-xs text-zinc-400">WhatsApp Group Intelligence Logbook</p>
            </div>
          </div>
        ...
        <div className="flex px-6 pt-4 border-b border-white/10 gap-6 print:hidden">
          <button 
            ...
            className={`pb-4 text-sm font-bold uppercase tracking-wider border-b-2 transition-colors ${activeTab === 'logs' ? 'border-emerald-500 text-emerald-400' : 'border-transparent text-zinc-500 hover:text-zinc-300'}`}
          >
            📖 Logbook
          </button>
          <button 
            onClick={() => setActiveTab('targets')}
            className={`pb-4 text-sm font-bold uppercase tracking-wider border-b-2 transition-colors ${activeTab === 'targets' ? 'border-emerald-500 text-emerald-400' : 'border-transparent text-zinc-500 hover:text-zinc-300'}`}
          >
            🎯 WAG Targets
          </button>
          <button 
        ...
        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar print:p-0 print:overflow-visible">
          {loading ? (
            ...
            <>
              {/* DELIVERY TAB */}
              {activeTab === 'delivery' && (
                <AutoDeliverySettings providerId={providerId} logType="emailGroup" />
              )}

              {/* ANALYTICS TAB */}
              {activeTab === 'analytics' && (
                <LogAnalytics logs={filteredLogs} targets={targets} type="emailGroup" />
              )}
              ...
              {activeTab === 'targets' && (
                <div className="space-y-6 print:hidden">
                  <div className="bg-zinc-900/50 border border-white/5 rounded-2xl p-6">
                    <h3 className="text-sm font-bold text-white mb-4 uppercase tracking-widest">Add New Group Target</h3>
                    <form onSubmit={handleAddTarget} className="flex gap-4 items-end">
                      <div className="flex-1 space-y-2">
                        <label className="text-xs text-zinc-400 uppercase tracking-wider">Group JID (ID Grup)</label>
                        <input type="text" value={groupId} onChange={e=>setGroupId(e.target.value)} placeholder="1234567890@g.us" className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:ring-1 focus:ring-emerald-500 focus:outline-none" required />
                      </div>
                      <div className="flex-1 space-y-2">
                    ...
                    <div className="p-4 border-b border-white/5 bg-zinc-900/30 flex flex-col gap-4">
                      <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                        {selectedTargetRows.length > 0 ? (
                          <div className="bg-emerald-900/20 px-4 py-2 border border-emerald-500/20 flex items-center gap-4 rounded-lg w-full justify-between">
                            <span className="text-sm font-bold text-emerald-400">{selectedTargetRows.length} targets selected</span>
                      ...
                      <div className="relative w-full">
                        ...
                        <input 
                          type="text" 
                          placeholder="Cari target WAG berdasarkan Nama atau ID Grup..." 
                          value={searchTargetQuery}
                          onChange={e=>setSearchTargetQuery(e.target.value)}
                          className="w-full bg-black/60 border border-emerald-500/30 rounded-xl pl-12 pr-4 py-3 text-sm text-white focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all placeholder:text-zinc-600 shadow-inner"
                        />
                      </div>
                    </div>
                      ...
                      <thead className="bg-zinc-900/80 text-xs uppercase text-zinc-500">
                        <tr>
                          <th className="px-6 py-4 w-10">
                            <input type="checkbox" onChange={handleSelectAllTargets} checked={targets.length > 0 && selectedTargetRows.length === targets.length} className="w-4 h-4 rounded bg-black/40 border-white/10 text-emerald-500 focus:ring-emerald-500" />
                          </th>
                          <th className="px-6 py-4">Group Name</th>
                          <th className="px-6 py-4">Group JID</th>
                          <th className="px-6 py-4">Added On</th>
                          <th className="px-6 py-4 text-right">Actions</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-white/5">
                        {targets.filter(t => t.groupName?.toLowerCase().includes(searchTargetQuery.toLowerCase()) || t.groupId?.includes(searchTargetQuery)).map(t => (
                          <tr key={t.id} className={`hover:bg-white/[0.02] ${selectedTargetRows.includes(t.id) ? 'bg-emerald-500/5' : ''}`}>
                            <td className="px-6 py-4">
                              <input type="checkbox" checked={selectedTargetRows.includes(t.id)} onChange={() => toggleTargetRow(t.id)} className="w-4 h-4 rounded bg-black/40 border-white/10 text-emerald-500 focus:ring-emerald-500" />
                            </td>
                        ...
                        {targets.length === 0 && (
                          <tr><td colSpan={4} className="px-6 py-8 text-center text-zinc-500 italic">No WAG targets monitored.</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
                  ...
                  <div className="flex flex-col gap-4 print:hidden">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <div className="flex items-center gap-2 ml-2">
                          ...
                          {(filterDateFrom || filterDateTo) && (
                            <button onClick={() => { setFilterDateFrom(""); setFilterDateTo(""); }} className="px-3 py-1.5 bg-emerald-600/20 hover:bg-emerald-600/40 text-emerald-400 border border-emerald-500/30 rounded-lg text-xs font-bold transition-colors">
                              All Time
                            </button>
                          )}
                        </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        <ImportMenu type="emailGroup" isLoading={importing} onImport={handleImportData} />
                    ...
                    <div className="relative w-full">
                      ...
                      <input 
                        type="text" 
                        placeholder="Ketik untuk mencari log pesan WAG..." 
                        value={searchQuery}
                        onChange={e=>setSearchQuery(e.target.value)}
                        className="w-full bg-black/60 border border-emerald-500/30 rounded-xl pl-12 pr-4 py-4 text-sm text-white focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all placeholder:text-zinc-600 shadow-inner"
                      />
                    </div>
                  </div>
                                  ...
                                  <div className="flex flex-col gap-2 print:hidden">
                                    {l.mediaType === 'image' ? (
                                      <div className="relative group">
                                        <img src={l.mediaUrl} alt="WAG Media" className="w-24 h-24 object-cover rounded-lg border border-white/10" />
