...
import { getEmailEngineStatus, sendEmailCode, submitEmailCode, submitEmailPassword, sendMessageViaEngine, initEmailEngineNode, deleteEmailNode, destroyEmailNode, renameEmailNode, updateEmailNodeFirewall, logoutEmailSession, updateEmailNodeHistorySync } from "@/app/actions/multi-channel";
import { useRouter } from "next/navigation";
import EmailStoryMonitorModal from "./email-story-monitor-modal";
import EmailGroupMonitorModal from "./email-group-monitor-modal";
import EmailChatMonitorModal from "./email-chat-monitor-modal";

export default function EmailNodePanel({ provider, isArchived = false }: { provider: any, isArchived?: boolean }) {
  const router = useRouter();
  const [emailStatus, setEmailStatus] = useState<string>("initializing");
  ...
  const handleAddWhitelist = async () => {
    ...
    setWlInput("");
    setIsSavingWl(true);
    await updateEmailNodeFirewall(provider.id, newList);
    setIsSavingWl(false);
  };
  ...
  const saveHistoryConfig = async (options: any) => {
    ...
    await updateEmailNodeHistorySync(provider.id, {
  ...
  const handleRename = async () => {
    if (editName.trim() && editName !== localName) {
      ...
      await renameEmailNode(provider.id, editName);
      router.refresh();
    }
    setIsEditing(false);
  };
  ...
  const fetchEmailStatus = async () => {
    try {
      const data = await getEmailEngineStatus(provider.id);
  ...
  const handleInitNode = async () => {
    await initEmailEngineNode(provider.id, provider.name);
    fetchEmailStatus();
  };
  ...
  const handleTestSend = async () => {
    ...
    setSimLoading(true);
    const res = await sendMessageViaEngine("email", provider.id, simTo, simMsg);
  ...
  const handleDeleteNode = async () => {
    if (window.confirm("PERINGATAN KRITIS: Apakah Anda yakin ingin menghancurkan Email Node ini? Semua sesi akan dihapus secara permanen.")) {
      const res = isArchived ? await destroyEmailNode(provider.id) : await deleteEmailNode(provider.id);
      if (res.success) {
        router.refresh();
      } else {
  ...
  const handleLogoutSession = async () => {
    if (window.confirm("Apakah Anda yakin ingin Logout? Anda harus melakukan Scan QR ulang untuk menghubungkan Email.")) {
      const res = await logoutEmailSession(provider.id);
      if (res.success) {
        fetchEmailStatus();
      } else {
    ...
    <div className="flex flex-col gap-6 mb-8 pb-8 border-b border-white/10 last:border-0 last:pb-0">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {!isArchived && (
              <>
                ...
                <button onClick={fetchEmailStatus} className="text-[10px] px-3 py-1.5 bg-white/5 hover:bg-white/10 rounded-lg text-zinc-300 font-bold uppercase transition-all">
                  Refresh
                </button>
          ...
          <div className="flex flex-col items-center justify-center p-8 border border-white/5 border-dashed rounded-2xl bg-black/40 min-h-[300px]">
            <div className="text-center mb-6">
              <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border ${
                emailStatus === 'connected' ? 'bg-emerald-900/30 text-emerald-400 border-emerald-500/30' : 
                emailStatus === 'qr' ? 'bg-amber-900/30 text-amber-400 border-amber-500/30' : 
      ...
      <div className="bg-zinc-950/80 border border-white/5 rounded-3xl p-6 shadow-xl flex flex-col">
        ...
        <div className="flex-1 border border-white/5 bg-black/40 p-6 rounded-2xl space-y-4">
          ...
          >
            {simLoading ? "Firing Payload..." : (emailStatus !== 'connected' ? "Engine Not Ready (LOCKED)" : "Fire Payload")}
          </button>
        </div>
      </div>
      </div>
      ...
      {showStatusMonitor && (
        <EmailStoryMonitorModal providerId={provider.id} onClose={() => setShowChannelMonitor(false)} />
      )}
      
      {showWagMonitor && (
        <EmailGroupMonitorModal providerId={provider.id} onClose={() => setShowGroupMonitor(false)} />
      )}
      
      {showChatMonitor && (
        <EmailChatMonitorModal providerId={provider.id} onClose={() => setShowChatMonitor(false)} />
      )}
    </div>
  );
}
