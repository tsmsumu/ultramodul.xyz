...
import { getMonitorTargets, getGroupLogs, addGroupTarget, removeGroupTarget, bulkDeleteLogs, bulkArchiveLogs, bulkDeleteTargets, importLogbookData } from "@/app/actions/sig-monitor";
...
export default function SigGroupMonitorModal({ providerId, onClose }: { providerId: string, onClose: () => void }) {
  ...
  const fetchData = async () => {
    setLoading(true);
    const { sigGroup } = await getMonitorTargets(providerId);
    setTargets(sigGroup);
  ...
  const handleBulkDelete = async () => {
    ...
    if (window.confirm(`Delete ${selectedRows.length} logs permanently?`)) {
      await bulkDeleteLogs('sigGroup', selectedRows);
      setSelectedRows([]);
      fetchData();
    }
  };
  ...
  const handleBulkArchive = async (archive: boolean) => {
    if (selectedRows.length === 0) return;
    await bulkArchiveLogs('sigGroup', selectedRows, archive);
    setSelectedRows([]);
    fetchData();
  };
  ...
  const handleBulkDeleteTargets = async () => {
    ...
    if (window.confirm(`Delete ${selectedTargetRows.length} WAG targets and all their associated logs? This action cannot be undone.`)) {
      await bulkDeleteTargets('sigGroup', selectedTargetRows);
  ...
  const handleImportData = async (data: any[]) => {
    setImporting(true);
    const res = await importLogbookData(providerId, 'sigGroup', data);
    if (res.success) {
        ...
        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar print:p-0 print:overflow-visible">
          {loading ? (
            ...
            <>
              {/* DELIVERY TAB */}
              {activeTab === 'delivery' && (
                <AutoDeliverySettings providerId={providerId} logType="sigGroup" />
              )}

              {/* ANALYTICS TAB */}
              {activeTab === 'analytics' && (
                <LogAnalytics logs={filteredLogs} targets={targets} type="sigGroup" />
              )}
                  ...
                  <div className="flex flex-col gap-4 print:hidden">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        <ImportMenu type="sigGroup" isLoading={importing} onImport={handleImportData} />
