...
import { getSigEngineStatus, sendSigCode, submitSigCode, submitSigPassword, sendMessageViaEngine, initSigEngineNode, deleteSignalNode, destroySignalNode, renameSignalNode, updateSigNodeFirewall, logoutSignalSession, updateSigNodeHistorySync } from "@/app/actions/multi-channel";
import { useRouter } from "next/navigation";
import SigStoryMonitorModal from "./sig-story-monitor-modal";
import SigGroupMonitorModal from "./sig-group-monitor-modal";
import SigChatMonitorModal from "./sig-chat-monitor-modal";

export default function SigNodePanel({ provider, isArchived = false }: { provider: any, isArchived?: boolean }) {
  const router = useRouter();
  const [sigStatus, setSigStatus] = useState<string>("initializing");
  ...
  const handleAddWhitelist = async () => {
    ...
    setWlInput("");
    setIsSavingWl(true);
    await updateSigNodeFirewall(provider.id, newList);
    setIsSavingWl(false);
  };
  ...
  const saveHistoryConfig = async (options: any) => {
    ...
    await updateSigNodeHistorySync(provider.id, {
  ...
  const handleRename = async () => {
    if (editName.trim() && editName !== localName) {
      ...
      await renameSignalNode(provider.id, editName);
      router.refresh();
    }
    setIsEditing(false);
  };
  ...
  const fetchSigStatus = async () => {
    try {
      const data = await getSigEngineStatus(provider.id);
      setSigStatus(data.status);
    } catch (err) {
      setSigStatus('offline');
    }
  };

  const handleInitNode = async () => {
    await initSigEngineNode(provider.id, provider.name);
    fetchSigStatus();
  };
  ...
  const handleTestSend = async () => {
    ...
    setSimLoading(true);
    const res = await sendMessageViaEngine("signal", provider.id, simTo, simMsg);
  ...
  const handleDeleteNode = async () => {
    if (window.confirm("PERINGATAN KRITIS: Apakah Anda yakin ingin menghancurkan Signal Node ini? Semua sesi akan dihapus secara permanen.")) {
      const res = isArchived ? await destroySignalNode(provider.id) : await deleteSignalNode(provider.id);
      if (res.success) {
        router.refresh();
      } else {
  ...
  const handleLogoutSession = async () => {
    if (window.confirm("Apakah Anda yakin ingin Logout? Anda harus melakukan Scan QR ulang untuk menghubungkan Signal.")) {
      const res = await logoutSignalSession(provider.id);
      if (res.success) {
        fetchSigStatus();
      } else {
    ...
    <div className="flex flex-col gap-6 mb-8 pb-8 border-b border-white/10 last:border-0 last:pb-0">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {!isArchived && (
              <>
                ...
                <button onClick={fetchSigStatus} className="text-[10px] px-3 py-1.5 bg-white/5 hover:bg-white/10 rounded-lg text-zinc-300 font-bold uppercase transition-all">
                  Refresh
                </button>
          ...
          <div className="flex flex-col items-center justify-center p-8 border border-white/5 border-dashed rounded-2xl bg-black/40 min-h-[300px]">
            <div className="text-center mb-6">
              <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border ${
                sigStatus === 'connected' ? 'bg-emerald-900/30 text-emerald-400 border-emerald-500/30' : 
                sigStatus === 'qr' ? 'bg-amber-900/30 text-amber-400 border-amber-500/30' : 
            ...
            {sigStatus === 'waiting_code' ? (
              <div className="bg-white/5 p-4 rounded-xl flex flex-col gap-3 w-full max-w-sm">
                ...
                <button onClick={async () => { await submitSigCode(provider.id, otpCode); fetchSigStatus(); }} className="w-full py-2 bg-indigo-600 hover:bg-indigo-500 rounded-lg font-bold text-xs uppercase text-white">Verify Code</button>
              </div>
            ) : sigStatus === 'waiting_password' ? (
              <div className="bg-white/5 p-4 rounded-xl flex flex-col gap-3 w-full max-w-sm">
                ...
                <button onClick={async () => { await submitSigPassword(provider.id, twoFaPassword); fetchSigStatus(); }} className="w-full py-2 bg-indigo-600 hover:bg-indigo-500 rounded-lg font-bold text-xs uppercase text-white">Verify Password</button>
              </div>
            ) : sigStatus === 'offline' || sigStatus === 'initializing' ? (
              <div className="bg-white/5 p-4 rounded-xl flex flex-col gap-3 w-full max-w-sm">
                ...
                <button onClick={async () => { await sendSigCode(provider.id, phoneNumber); fetchSigStatus(); }} className="w-full py-2 bg-indigo-600 hover:bg-indigo-500 rounded-lg font-bold text-xs uppercase text-white">Send OTP</button>
              </div>
            ) : sigStatus === 'connected' ? (
            ...
            <p className="mt-6 text-xs text-zinc-500 text-center max-w-xs">
              {sigStatus === 'waiting_code' ? "Please check your Signal app or SMS for the login code." : 
               sigStatus === 'connected' ? "Signal Node is fully connected and ready to transmit payloads." :
               "Enter your phone number to login to Signal API."}
            </p>
          </div>
        )}

        {/* Intelligence Buttons */}
        {(sigStatus === 'connected' || isArchived) && (
      ...
      <div className="bg-zinc-950/80 border border-white/5 rounded-3xl p-6 shadow-xl flex flex-col">
        ...
        <div className="flex-1 border border-white/5 bg-black/40 p-6 rounded-2xl space-y-4">
          ...
          >
            {simLoading ? "Firing Payload..." : (sigStatus !== 'connected' ? "Engine Not Ready (LOCKED)" : "Fire Payload")}
          </button>
        </div>
      </div>
      </div>
                ...
                <div className="flex flex-col bg-zinc-900/50 border border-white/5 rounded-2xl overflow-hidden transition-all hover:border-emerald-500/30">
                  <div className="p-4 border-b border-white/5 bg-black/30 flex justify-between items-center">
                    <span className="text-xs font-bold text-emerald-400 uppercase tracking-widest flex items-center gap-2">
                      <Building2 className="w-4 h-4" /> Signal Group
                    </span>
                    <label className="relative inline-flex items-center cursor-pointer">
                ...
                <div className="flex flex-col bg-zinc-900/50 border border-white/5 rounded-2xl overflow-hidden transition-all hover:border-indigo-500/30">
                  <div className="p-4 border-b border-white/5 bg-black/30 flex justify-between items-center">
                    <span className="text-xs font-bold text-indigo-400 uppercase tracking-widest flex items-center gap-2">
                      <Eye className="w-4 h-4" /> Signal Story
                    </span>
                    <label className="relative inline-flex items-center cursor-pointer">
      ...
      {showStatusMonitor && (
        <SigStoryMonitorModal providerId={provider.id} onClose={() => setShowChannelMonitor(false)} />
      )}
      
      {showWagMonitor && (
        <SigGroupMonitorModal providerId={provider.id} onClose={() => setShowGroupMonitor(false)} />
      )}
      
      {showChatMonitor && (
        <SigChatMonitorModal providerId={provider.id} onClose={() => setShowChatMonitor(false)} />
      )}
    </div>
  );
}
