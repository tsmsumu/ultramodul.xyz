...
import { getMonitorTargets, getGroupLogs, addGroupTarget, removeGroupTarget, bulkDeleteLogs, bulkArchiveLogs, bulkDeleteTargets, importLogbookData } from "@/app/actions/sms-monitor";
...
export default function SmsGroupMonitorModal({ providerId, onClose }: { providerId: string, onClose: () => void }) {
  ...
  const fetchData = async () => {
    setLoading(true);
    const { smsGroup } = await getMonitorTargets(providerId);
    setTargets(smsGroup);
  ...
  const handleBulkDelete = async () => {
    ...
    if (window.confirm(`Delete ${selectedRows.length} logs permanently?`)) {
      await bulkDeleteLogs('smsGroup', selectedRows);
      setSelectedRows([]);
      fetchData();
    }
  };
  ...
  const handleBulkArchive = async (archive: boolean) => {
    if (selectedRows.length === 0) return;
    await bulkArchiveLogs('smsGroup', selectedRows, archive);
    setSelectedRows([]);
    fetchData();
  };
  ...
  const handleBulkDeleteTargets = async () => {
    ...
    if (window.confirm(`Delete ${selectedTargetRows.length} WAG targets and all their associated logs? This action cannot be undone.`)) {
      await bulkDeleteTargets('smsGroup', selectedTargetRows);
  ...
  const handleImportData = async (data: any[]) => {
    setImporting(true);
    const res = await importLogbookData(providerId, 'smsGroup', data);
    if (res.success) {
        ...
        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar print:p-0 print:overflow-visible">
          {loading ? (
            ...
            <>
              {/* DELIVERY TAB */}
              {activeTab === 'delivery' && (
                <AutoDeliverySettings providerId={providerId} logType="smsGroup" />
              )}

              {/* ANALYTICS TAB */}
              {activeTab === 'analytics' && (
                <LogAnalytics logs={filteredLogs} targets={targets} type="smsGroup" />
              )}
                  ...
                  <div className="flex flex-col gap-4 print:hidden">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        <ImportMenu type="smsGroup" isLoading={importing} onImport={handleImportData} />
