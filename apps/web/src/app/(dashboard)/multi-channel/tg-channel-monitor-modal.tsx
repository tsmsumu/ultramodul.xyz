...
import { getMonitorTargets, getChannelLogs, addChannelTarget, removeChannelTarget, bulkDeleteLogs, bulkArchiveLogs, bulkDeleteTargets, importLogbookData } from "@/app/actions/tg-monitor";
...
export default function ChannelMonitorModal({ providerId, onClose }: { providerId: string, onClose: () => void }) {
  ...
  const fetchData = async () => {
    ...
    setTargets(status);
    const logData = await getChannelLogs(providerId);
    setLogs(logData);
    setLoading(false);
  };
  ...
  const handleAddTarget = async (e: React.FormEvent) => {
    ...
    await addChannelTarget(providerId, phone, name, isTextOnly);
    setPhone("");
    setName("");
    setIsTextOnly(false);
    fetchData();
  ...
  const handleRemoveTarget = async (id: string) => {
    await removeChannelTarget(providerId, id);
    fetchData();
  };
  ...
  const handleBulkDelete = async () => {
    ...
    if (window.confirm(`Delete ${selectedRows.length} logs permanently?`)) {
      await bulkDeleteLogs('status', selectedRows);
      setSelectedRows([]);
      fetchData();
    }
  };
  ...
  const handleBulkArchive = async (archive: boolean) => {
    if (selectedRows.length === 0) return;
    await bulkArchiveLogs('status', selectedRows, archive);
    setSelectedRows([]);
    fetchData();
  };
  ...
  const handleBulkDeleteTargets = async () => {
    ...
    if (window.confirm(`Delete ${selectedTargetRows.length} Status targets and all their associated logs? This action cannot be undone.`)) {
      await bulkDeleteTargets('status', selectedTargetRows);
  ...
  const handleImportData = async (data: any[]) => {
    setImporting(true);
    const res = await importLogbookData(providerId, 'status', data);
    if (res.success) {
  ...
  const exportOptions = {
    ...
    title: "Omni Intelligence - Telegram Channel Monitor",
    columns: [
      ...
      <div className="bg-zinc-950 border border-white/10 rounded-3xl w-full max-w-5xl h-[85vh] flex flex-col overflow-hidden shadow-2xl print:h-auto print:border-none print:shadow-none">
        ...
        <div className="p-6 border-b border-white/10 flex justify-between items-center bg-zinc-900/50 print:hidden">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-white">Telegram Channel Monitor</h2>
                  ...
                  <div className="bg-black/40 border border-white/5 rounded-2xl overflow-hidden">
                    <div className="p-4 border-b border-white/5 bg-zinc-900/30 flex flex-col gap-4">
                      ...
                      <div className="relative w-full">
                        ...
                        <input 
                          type="text" 
                          placeholder="Cari target Telegram Channel berdasarkan Nama atau Nomor..." 
                ...
                <div className="space-y-4">
                  <div className="flex flex-col gap-4 print:hidden">
                    ...
                    <div className="relative w-full">
                      ...
                      <input 
                        type="text" 
                        placeholder="Ketik untuk mencari log Telegram Channel..." 
                        value={searchQuery}
