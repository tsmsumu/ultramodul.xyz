...
export default function NexusRecoveryPage() {
  ...
  const handleRollback = async () => {
    ...
    if (res.success) {
      alert("Loncatan Kuantum Berhasil! Mesin berhasil diselaraskan dengan masa lalu.");
      setRollbackTarget(null);
      setRollbackConfirm("");
      await loadTimeline();
    } else {
      alert("Gagal Beralih Dimensi: " + res.error);
    }
    setIsProcessing(false);
  };
  ...
  const handleUploadBackup = async () => {
    ...
    try {
      ...
      if (resp.ok) {
        alert("Restorasi Kuantum Eksternal Selesai! Mesin telah dipulihkan menggunakan File Eksternal Anda.");
        setUploadOpen(false);
        setSelectedFile(null);
        await loadTimeline();
      } else {
        alert("Terjadi kesalahan saat mesin mencoba mengekstrak file ZIP Eksternal.");
      }
    } catch(e) {
      alert("Gagal menghubungi pangkalan restorasi.");
    }
    setIsProcessing(false);
  };

  if (loading) {
     return <div className="p-10 flex text-emerald-500 justify-center flex-col items-center gap-4"><Loader2 className="w-10 h-10 animate-spin" /><span className="font-mono tracking-widest text-sm uppercase">Menghubungkan Garis Waktu...</span></div>;
  }
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <p className="text-gray-400 font-mono tracking-wider text-sm max-w-xl">
              Fasilitas Navigasi Non-Destructive Time Travel. Rollback ke sejarah manapun tanpa takut kehilangan jejak utama. Seluruh aksi bersifat Reversible.
            </p>
          </div>
          ...
          >
             <UploadCloud className="w-5 h-5" /> Unggah Snapshot Kustom
          </button>
        </div>
      </div>
                 ...
                 <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                    <div className="flex-1 space-y-3">
                       <div className="flex items-center gap-3">
                         ...
                         {isLatest && <span className="px-2 py-0.5 text-[10px] font-black uppercase text-black bg-emerald-500 rounded tracking-widest">Kondisi Saat Ini (HEAD)</span>}
                       </div>
                       ...
                       {point.note ? (
                         <div className="mt-4 p-4 bg-yellow-950/20 border border-yellow-900/50 rounded-lg">
                           <div className="flex items-center justify-between mb-2">
                             <span className="text-[10px] font-black tracking-widest text-yellow-600 uppercase">Catatan Abadi</span>
                             <button onClick={() => { setEditingNoteId(point.hash); setNoteDraft(point.note); }} className="text-[10px] text-yellow-600 hover:text-yellow-400 tracking-widest uppercase underline">Ubah</button>
                           </div>
                           <p className="text-sm text-yellow-200/80 font-mono leading-relaxed">{point.note}</p>
                         </div>
                       ) : (
                         editingNoteId !== point.hash && (
                           <button onClick={() => setEditingNoteId(point.hash)} className="mt-2 text-xs font-mono tracking-widest uppercase text-emerald-600 hover:text-emerald-400">+ Lekatkan Catatan Ingatan</button>
                         )
                       )}
                       ...
                       {editingNoteId === point.hash && (
                         <div className="mt-4 flex gap-2">
                           <textarea 
                             ...
                             placeholder="Contoh: Ini versi super aman sebelum instalasi Modul 3..."
                    ...
                    <div className="flex sm:flex-col gap-2 shrink-0 border-t border-gray-800 pt-4 md:border-t-0 md:pt-0 md:border-l md:pl-4">
                       <button onClick={() => triggerDownload(point.hash)} className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2 bg-gray-800 hover:bg-gray-700 text-gray-200 text-xs font-black uppercase tracking-widest rounded-lg transition border border-gray-700">
                         <Download className="w-4 h-4" /> BUNGKUS (.ZIP)
                       </button>
                       {!isLatest && (
                         <button onClick={() => setRollbackTarget(point.hash)} className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2 bg-red-950/30 hover:bg-red-900 border border-red-900/50 text-red-500 hover:text-red-300 text-xs font-black uppercase tracking-widest rounded-lg transition">
                           <ShieldAlert className="w-4 h-4" /> ROLLBACK KE SINI
                         </button>
                       )}
                    </div>
                 </div>
          ...
          <motion.div initial={{opacity: 0}} animate={{opacity: 1}} exit={{opacity: 0}} className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
             <motion.div initial={{scale: 0.9}} animate={{scale: 1}} exit={{scale: 0.9}} className="bg-zinc-950 border border-red-900/50 rounded-2xl max-w-lg w-full p-6 shadow-[0_0_50px_-10px_rgba(220,38,38,0.2)]">
               <div className="flex items-center gap-3 text-red-500 mb-4">
                 ...
                 <h2 className="text-xl font-black uppercase tracking-widest">Otorisasi Hukuman Mati Kode</h2>
               </div>
               <p className="text-gray-400 text-sm leading-relaxed mb-6">
                 Bapak akan mengubah tatanan ruang dan waktu dari pembuluh darah Server. Semua konfigurasi akan kembali persis ke Hash <span className="font-mono text-emerald-500">{rollbackTarget.substring(0,7)}</span>. Ketik <span className="font-bold text-red-500 select-all">SAYA YAKIN</span> pada matriks di bawah untuk melanjutkan.
               </p>
               <input 
                 type="text" 
                 placeholder="Ketik persetujuan Anda..." 
               ...
               <div className="flex flex-col-reverse sm:flex-row justify-end gap-3">
                 <button disabled={isProcessing} onClick={() => {setRollbackTarget(null); setRollbackConfirm("")}} className="px-6 py-3 font-bold text-sm tracking-widest uppercase text-gray-500 hover:text-white transition">Gemetar (Batal)</button>
                 <button disabled={rollbackConfirm !== "SAYA YAKIN" || isProcessing} onClick={handleRollback} className="flex items-center justify-center gap-2 px-6 py-3 bg-red-600 hover:bg-red-500 text-white rounded-xl font-bold text-sm tracking-widest uppercase transition disabled:opacity-20 shadow-[0_0_20px_-5px_rgba(220,38,38,0.6)]">
          ...
          <motion.div initial={{opacity: 0}} animate={{opacity: 1}} exit={{opacity: 0}} className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
             <motion.div initial={{y: 20}} animate={{y: 0}} exit={{y: 20}} className="bg-zinc-950 border border-amber-900/50 rounded-2xl max-w-lg w-full p-6 shadow-[0_0_50px_-10px_rgba(245,158,11,0.2)]">
               <div className="flex justify-between items-center mb-6">
                 <h2 className="text-xl font-black uppercase tracking-widest text-amber-500 flex items-center gap-2"><UploadCloud className="w-6 h-6"/> Sinkronisasi Manual</h2>
               ...
               <p className="text-gray-400 text-sm mb-6 leading-relaxed">
                 Fitur dewa tingkat akhir. Pilih arsip bundel (.ZIP) yang sebelumnya pernah Bapak unduh dari sistem ini. Semua isi VPS akan digilas ulang dan dibaptis dengan isi ZIP tersebut.
               </p>
               ...
               <div className="border-2 border-dashed border-amber-900/50 rounded-xl p-8 mb-6 flex flex-col items-center justify-center gap-4 hover:bg-amber-950/10 transition cursor-pointer" onClick={() => document.getElementById('snapshot-upload')?.click()}>
                 ...
                 <div className="text-center">
                   <p className="font-bold text-gray-300">Pilih Mahkota Cadangan (.ZIP)</p>
                   <p className="text-xs text-gray-600 mt-1">{selectedFile?.name || 'Maksimal ukuran ditentukan RAM Peladen'}</p>
                 </div>
               ...
               <div className="flex justify-end gap-3 border-t border-gray-800 pt-6">
                 <button disabled={isProcessing} onClick={() => setUploadOpen(false)} className="px-6 py-3 font-bold text-sm tracking-widest uppercase text-gray-500 hover:text-white transition">Batal Mengudara</button>
                 <button disabled={!selectedFile || isProcessing} onClick={handleUploadBackup} className="flex items-center gap-2 px-6 py-3 bg-amber-500 hover:bg-amber-400 text-black rounded-xl font-black tracking-widest uppercase transition disabled:opacity-50">
                    {isProcessing ? <Loader2 className="w-4 h-4 animate-spin" /> : "LEDAKKAN FILE ZIP"}
                 </button>
               </div>
             </motion.div>
          </motion.div>
