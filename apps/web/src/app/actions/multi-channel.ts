...
// --- EMAIL NODE ACTIONS ---
export async function createEmailNode() {
  try {
    const newId = randomUUID();
    await db.insert(mcProviders).values({
      id: newId,
      providerType: "email",
      name: `Email Node - ${newId.split('-')[0]}`,
      isActive: false,
      configPayload: "{}",
      createdAt: new Date(),
      updatedAt: new Date()
...
export async function deleteEmailNode(providerId: string) {
  try {
    try {
      await fetch(`http://127.0.0.1:3003/logout/${providerId}`, { method: 'POST' });
    } catch (e) {
...
export async function destroyEmailNode(providerId: string) {
  try {
    try {
      await fetch(`http://127.0.0.1:3003/logout/${providerId}`, { method: 'POST' });
    } catch (e) {
...
export async function renameEmailNode(providerId: string, name: string) {
  try {
    await db.update(mcProviders).set({ name, updatedAt: new Date() }).where(eq(mcProviders.id, providerId));
    try {
      await fetch(`http://127.0.0.1:3003/rename/${providerId}`, {
...
export async function initEmailEngineNode(providerId: string, name: string) {
  try {
    const res = await fetch(`http://127.0.0.1:3003/init/${providerId}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export async function getEmailEngineStatus(providerId: string) {
  try {
    const res = await fetch(`http://127.0.0.1:3003/status/${providerId}`);
    return await res.json();
  } catch (e) {
...
export async function sendEmailCode(providerId: string, phoneNumber: string) {
  try {
    const res = await fetch(`http://127.0.0.1:3003/auth/send-code/${providerId}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export async function submitEmailCode(providerId: string, code: string) {
  try {
    const res = await fetch(`http://127.0.0.1:3003/auth/submit-code/${providerId}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export async function submitEmailPassword(providerId: string, password: string) {
  try {
    const res = await fetch(`http://127.0.0.1:3003/auth/submit-password/${providerId}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export async function logoutEmailSession(providerId: string) {
  try {
    const res = await fetch(`http://127.0.0.1:3003/logout/${providerId}`, { method: 'POST' });
    return await res.json();
  } catch (e) {
...
export async function updateEmailNodeFirewall(providerId: string, whitelist: string[]) {
  try {
    await fetch(`http://127.0.0.1:3003/config/${providerId}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export async function updateEmailNodeHistorySync(providerId: string, payload: any) {
  try {
    await fetch(`http://127.0.0.1:3003/config/${providerId}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
